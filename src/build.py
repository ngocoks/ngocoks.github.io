...
def load_custom_css():
    """Mengembalikan string kosong agar tidak ada CSS kustom yang dimuat."""
    # Ini adalah versi diagnostik tanpa CSS kustom
    return ""
...
def build_index_and_label_pages(all_published_posts_data, all_unique_labels_list):
    ...
    for post in all_published_posts_data[:5]:
    ...
    for post in all_published_posts_data:
        ...
        # HTML SANGAT MINIMALIS UNTUK INDEX
        list_items_html_for_index.append(f"""
        <div style="margin-bottom: 15px; padding: 10px; border: 1px solid #eee;">
            <h3 style="margin-top: 0;"><a href="{permalink_rel}" style="color: black; text-decoration: none;">{post['processed_title']}</a></h3>
            <p style="font-size: 0.9em; color: #555;">{snippet_for_index}</p>
            <p style="font-size: 0.8em; color: #777;">Published: {datetime.fromisoformat(post['published'].replace('Z', '+00:00')).strftime('%d %b, %Y')}</p>
        </div>
        """)
    ...
    <body>
        <h1 style="color: black;">{BLOG_NAME} - Beranda</h1>
        <p style="color: black;">Selamat datang di blog kami! Ini adalah daftar artikel terbaru.</p>
        <div>
            {"".join(list_items_html_for_index)}
        </div>
        <p style="font-size: 0.8em; color: #777;">&copy; {datetime.now().year} {BLOG_NAME}. All rights reserved.</p>
    </body>
    </html>
    """
    print(f"DEBUG: Length of generated index_html_content: {len(index_html_content)} bytes")
    ...
    for label_name in all_unique_labels_list:
        ...
        for post in posts_for_this_label:
            ...
            # HTML SANGAT MINIMALIS UNTUK LABEL
            list_items_html_label.append(f"""
            <div style="margin-bottom: 15px; padding: 10px; border: 1px solid #eee;">
                <h3 style="margin-top: 0;"><a href="{permalink_rel}" style="color: black; text-decoration: none;">{post['processed_title']}</a></h3>
                <p style="font-size: 0.9em; color: #555;">{snippet_for_label}</p>
                <p style="font-size: 0.8em; color: #777;">Published: {datetime.fromisoformat(post['published'].replace('Z', '+00:00')).strftime('%d %b, %Y')}</p>
            </div>
            """)
        ...
        <body>
            <h1 style="color: black;">Kategori: {label_name} - {BLOG_NAME}</h1>
            <div>
...
def build_single_post_page(post):
    ...
    print(f"DEBUG: temp_html_from_pure_text length: {len(temp_html_from_pure_text)} bytes")

    final_article_content_html = remove_anchor_tags_and_apply_replacements_to_html(temp_html_from_pure_text)
    print(f"DEBUG: final_article_content_html length: {len(final_article_content_html)} bytes")
    ...
    <head>
        ...
        <style>
            body {{ font-family: sans-serif; margin: 20px; color: black; }}
            h1 {{ color: black; }}
            p {{ line-height: 1.6; color: black; }}
        </style>
    </head>
    <body>
        <h1 style="color: black;">{post['processed_title']}</h1>
    ...
    """
    print(f"DEBUG: Total article_html length for '{post['processed_title']}': {len(article_html)} bytes")
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(article_html)

# --- Main Logic ---
if __name__ == "__main__":
    # --- BARIS BARU: Cek flag RESET_PUBLISHED_POSTS ---
    reset_flag = os.getenv("RESET_PUBLISHED_POSTS", "false").lower() == "true"
    print(f"DEBUG: reset_flag in Python (from env var): {reset_flag}")

    if reset_flag:
        print("🚨 RESET_PUBLISHED_POSTS is TRUE. Clearing published_posts.json and all_blogger_posts_cache.json.")
        # Pastikan folder dist ada sebelum mencoba menulis file cache
        # Note: published_posts.json dan all_blogger_posts_cache.json seharusnya ada di root, bukan dist
        # os.makedirs(os.path.dirname(PUBLISHED_LOG_FILE), exist_ok=True) # Ini tidak perlu jika di root
        # os.makedirs(os.path.dirname(ALL_BLOGGER_POSTS_CACHE_FILE), exist_ok=True) # Ini tidak perlu jika di root

        # Forcefully clear the files
        with open(PUBLISHED_LOG_FILE, 'w', encoding='utf-8') as f:
            json.dump([], f)
        ...
        # --- DEBUGGING: Konfirmasi status file setelah reset ---
        print(f"DEBUG: published_posts.json exists after reset: {os.path.exists(PUBLISHED_LOG_FILE)}")
        print(f"DEBUG: all_blogger_posts_cache.json exists after reset: {os.path.exists(ALL_BLOGGER_POSTS_CACHE_FILE)}")
        
        # Setelah clearing, coba muat ulang dan cek count-nya
        published_ids_after_reset = load_published_posts_state()
        all_posts_preprocessed_after_reset = load_json_cache(ALL_BLOGGER_POSTS_CACHE_FILE)
        print(f"DEBUG: published_ids count AFTER reset attempt: {len(published_ids_after_reset)}")
        print(f"DEBUG: all_posts_preprocessed count AFTER reset attempt: {len(all_posts_preprocessed_after_reset)}")
    ...
    if unpublished_posts:
        ...
        edited_title_by_gemini = post_to_process_with_gemini['original_title']
        try:
        ...
        post_to_process_with_gemini['processed_title'] = edited_title_by_gemini

        edited_pure_text_from_gemini_300_words = ""
        try:
            edited_pure_text_from_gemini_300_words = edit_first_300_words_with_gemini(
                post_to_process_with_gemini['id'],
                post_to_process_with_gemini['processed_title'],
                post_to_process_with_gemini['filtered_pure_text_for_gemini_input']
            )
        except Exception as e:
